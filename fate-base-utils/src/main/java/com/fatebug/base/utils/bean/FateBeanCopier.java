 ...
 *
 */
public abstract class FateBeanCopier {
	private static final Type CONVERTER = TypeUtils.parseType("org.springframework.cglib.core.Converter");
	private static final Type BEAN_COPIER = TypeUtils.parseType(FateBeanCopier.class.getName());
	...
	/**
	 * The map to store {@link FateBeanCopier} of source type and class type for copy.
	 */
	private static final ConcurrentMap<FateBeanCopierKey, FateBeanCopier> BEAN_COPIER_MAP = new ConcurrentHashMap<>();

	public static FateBeanCopier create(Class source, Class target, boolean useConverter) {
		return FateBeanCopier.create(source, target, useConverter, false);
	}

	public static FateBeanCopier create(Class source, Class target, boolean useConverter, boolean nonNull) {
		FateBeanCopierKey copierKey = new FateBeanCopierKey(source, target, useConverter, nonNull);
	...
	public static class Generator extends AbstractClassGenerator {
		private static final Source SOURCE = new Source(FateBeanCopier.class.getName());
		...
		protected ProtectionDomain getProtectionDomain() {
			return ReflectUtils.getProtectionDomain(sources);
		}

		@Override
		public FateBeanCopier create(Object key) {
			return (FateBeanCopier) super.create(key);
		}

		@Override
		public void generateClass(ClassVisitor v) {
			Type sourceType = Type.getType(sources);
			...
			// map 单独处理
			if (Map.class.isAssignableFrom(sources)) {
			...
			PropertyDescriptor[] getters = ReflectUtils.getBeanGetters(sources);
			...
			for (PropertyDescriptor setter : setters) {
				...
				CopyProperty targetIgnoreCopy = ClassUtil.getAnnotation(target, propName, CopyProperty.class);
				// set 上有忽略的 注解
