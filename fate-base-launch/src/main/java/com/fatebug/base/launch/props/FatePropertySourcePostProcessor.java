 ...
 */
@Slf4j
public class FatePropertySourcePostProcessor implements BeanFactoryPostProcessor, InitializingBean, Ordered {
	private final ResourceLoader resourceLoader;
	private final List<PropertySourceLoader> propertySourceLoaders;

	public FatePropertySourcePostProcessor() {
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		log.info("FatePropertySourcePostProcessor process @FatePropertySource bean.");
		Map<String, Object> beansWithAnnotation = beanFactory.getBeansWithAnnotation(FatePropertySource.class);
		Set<Map.Entry<String, Object>> beanEntrySet = beansWithAnnotation.entrySet();
		// 没有 @YmlPropertySource 注解，跳出
		if (beanEntrySet.isEmpty()) {
			log.warn("Not found @FatePropertySource on spring bean class.");
			return;
		}
		// 组装资源
		...
		for (Map.Entry<String, Object> entry : beanEntrySet) {
			...
			FatePropertySource propertySource = AnnotationUtils.getAnnotation(beanClass, FatePropertySource.class);
			if (propertySource == null) {
				continue;
			}
			int order = propertySource.order();
			boolean loadActiveProfile = propertySource.loadActiveProfile();
			String[] values = propertySource.values();
	...
	private static void loadPropertySource(String location, Resource resource,
		...
		if (resource.exists()) {
			String name = "fatePropertySource: [" + location + "]";
			try {
	...
	public void afterPropertiesSet() throws Exception {
		log.info("FatePropertySourcePostProcessor init.");
	}
